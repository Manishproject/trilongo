<section class="pg_mid">
    <div class="form_pages">
        <div class="company-edit">
            <div class="form-heading"><h2>Service Info Vehicle</h2></div>
            <div class="clear"></div>
            <?php
            echo $this->Form->create('ServiceInformation');
            echo $this->Session->flash('logmsg');
            $ServiceList = $this->Lab->get_service_vehicle();

            if (isset($ServiceList['vehicle_type']) && !empty($ServiceList['vehicle_type'])) {
                $i=0;
                foreach ($ServiceList['vehicle_type'] as $service_area_key => $service_area_value) {

                    ?>
                    <div class="service_info_box">
                        <strong><?php echo ucfirst($service_area_value['VehicleType']['name']) ?></strong>
                                ...
                                foreach ($ServiceList['service_type'] as $service_type_key => $service_type_value) {
                                    ...
                                    <div class="ser_res">
                                        ...
                                        if(isset($this->request->data[$i]['ServiceInformation']['id']) && !empty($this->request->data[$i]['ServiceInformation']['id'])){
                                         ...
                                         echo $this->Form->hidden($i.".ServiceInformation.service_id",array('label'=>false,'value'=>3));
                                         ...
                                         echo $this->Form->hidden($i.".ServiceInformation.vehicle_id",array('label'=>false,'value'=>$service_area_value['VehicleType']['id']));
...
<script>
    $(function(){
        $("#ServiceInformationServiceInfoVehicleForm").validationEngine({scroll: false});
    })
</script>