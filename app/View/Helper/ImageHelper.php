...
class ImageHelper extends Helper
{
    ...
    public function resize($path, $width, $height, $aspect = true, $htmlAttributes = array(), $return = false)
    {
        ...
        if ($size[0] > $width && $size[1] > $height) {
            $art = 'data-original=' . $relfile;
            //return $this->output(sprintf($this->Html->_tags['image'], $relfile, $this->Html->_parseAttributes($htmlAttributes, null, '', ' ')), $return);
            return $this->output(sprintf($this->Html->_tags['image'], $relfile, $this->Html->_parseAttributes($art, $htmlAttributes, null, '', ' ')), $return);
        } else {
            $new_path = SITEURL . $path;
            $art = 'data-original=' . $new_path;
            return $this->output(sprintf($this->Html->_tags['image'], $new_path, $this->Html->_parseAttributes($art, $htmlAttributes, null, '', ' ')), $return);
        }
